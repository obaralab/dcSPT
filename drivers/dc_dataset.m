...
for i = 1:numel(D.channels)
    ...
    if ~isempty(t)
        ...
        % Check the FINITE ones only. Mixing ~ok (full length) with loc(ok) (the subset) is a size
        % error the moment a dataset holds both tracked and untracked detections — which every real
        % one does, and no all-tracked or all-untracked fixture ever showed.
        assert(all(loc(ok) >= 0 & loc(ok) < D.tracks(t).nTracks), 'dc_dataset:badLocalTrack', ...
        ...
        assert(all(gl(ok) == loc(ok) + D.tracks(t).idOffset), 'dc_dataset:trackIdDrift', ...
