...
farS  = St(St.r >  rFar,  :);
nearS = St(St.r <= rNear, :);
% The far field is capped by dc_comotion's rMaxUm, so "beyond rFarUm" is really the shell between
% rFarUm and rMaxUm. A shell that is thin, or absent, is the usual reason this is empty — and saying
% "too few pairs" without saying why sends people to lower rFarUm when the fix is to raise rMaxUm.
rMax = Inf;
if isfield(R,'params') && isfield(R.params,'rMaxUm'), rMax = R.params.rMaxUm; end
assert(height(farS) >= 200, 'dc_comotion_null:thinFar', ...
    ['only %d step pairs beyond %.2f um to build the null from, and %d needed.\n' ...
     'The far field is the shell between rFarUm (%.2f) and dc_comotion''s rMaxUm (%.2f), which is ' ...
     '%.2f um wide%s. Either raise rMaxUm so distant pairs are kept at all, or lower rFarUm — but ' ...
     'not below the range where pairs might still be interacting, or the null absorbs the signal.'], ...
    height(farS), rFar, 200, rFar, rMax, max(rMax-rFar,0), ...
    tern(rMax - rFar < rFar/2, ' — narrow relative to rFarUm, which is very likely the problem', ''));
